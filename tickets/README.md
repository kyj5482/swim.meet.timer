...
| T-119 | UX 피드백 라운드 2: 공용 선수 헤더·탭 간 선수 공유·페이스 인사이트(로버스트)·세션 레벨 배지·전 코스 사다리 + AI 코치 로그인·웹 기록 뷰어 | done |
| T-207 | SplitLane Cloud 웹 포털: 역할 + 코치 리뷰 요청 | done(로컬 프리뷰·엔진; AWS 배포 잔여) |
| T-121 | Event Detail 라운드 3 + 웹 서비스(수영/진학 코치 AI·PII 암호화·correlation 로깅) | done |
...
| T-305 | AI 가이드 엔진: 수영 코치(수준·나이별 훈련/영상) + 진학 코치(학교 적합도) | done(결정적 엔진·테스트; Bedrock 문장화 잔여) |
| T-306 | 진학: 학교 평가 PDF 업로드 → 수학·ELA·AP 파싱(민감정보 암호화 저장) | todo(암호화 완료·PDF 파싱 잔여) |
...
| T-405 | 코치 리뷰 왕복(기록+영상 업로드) + 인앱 공지 → 웹 전환 | todo(웹 요청·매칭·수락 완료; 영상 S3·공지 잔여) |
| T-406 | 입시 코치 스토리 빌더(목표 대학·학업·기록·활동 타임라인) | todo |
| T-407 | 1:1 라이브 영상 코칭(모바일 영상통화, 원어민 1:1) — 웹 "Soon" | todo |
